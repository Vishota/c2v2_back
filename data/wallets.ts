 ...
 */
export async function pay(walletId: number, delta: number, description?:string, purchasedCourseId?: number) {
    const dbResponse = await db.query(`WITH updated as (
       ...
       [walletId, delta, description ? description : '', purchasedCourseId])
