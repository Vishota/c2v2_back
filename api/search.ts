...
export default {
    '/search/byprompt': async (req, res) => {
        const request = checkKeys(JSON.parse(req.body) as any, ['text']);
        const results = await courses.search(request.text);
        res.send({ results })
    },
    '/search/byowner': async (req, res) => {
        const request = checkKeys(JSON.parse(req.body) as any, ['teacherId']);
        const results = await courses.getOwnedBy(parseInt(request.teacherId));
        res.send({ results });
    },
    '/search/byaccess': async (req, res) => {
        const request = checkKeys(JSON.parse(req.body) as any, ['accountId']);
